...
class EasyCsv{
	...
	public static function OpenCSV($filename, $options=array()){
		...
		$csv=array_merge(
			array('rows'=>array()),
			$default,
			array_intersect_key($options, $default)
		);
		...
		if ($handle !== false) {
			...
			while(($data = fgetcsv($handle, 0, ",")) !== false) {
				...
				if(key_exists('header', $csv)&&count($csv['header'])==0){
					$c++;
					$csv['header']=$data;
					if($csv['length']==0)$csv['length']=count($data);
				}else{
					$c++;
					$csv['rows'][]=$data;
					$count=count($data);
					if($count>$csv['length']&&$csv['length']>0)throw new Exception('CSV file contians longer than expected: ('.$csv['length'].':'.$count.')');
				}
	...
	public static function GetFieldNames($csvOrFilename, $csvMetdata=false){
		...
		if(is_array($csv)){
			...
			if(key_exists('header', $csv))return $csv['header'];
	...
	public static function CountColumns($csv){
		if($csv['length']>0)return $csv['length'];
	...
	/**
	 * @param array $csv csv object
	 * @param int $index row index
	 * @param array $fieldNames ordered array of names to accociate cell values (optional will use csv)
	 * @return array an string indexed array
	 */
	public static function GetRow_ASSOC($csv, $index, $fieldNames=false){
		if(!$fieldNames)$fieldNames=$csv['header'];
	...
	private static function _combine($header, $row){
		...
		if(count($header)!=count($row)){
			throw new Exception('Expected number of fields to match header['.implode(',', $header).']:'.count($header).' row@?['.implode(',', $row).']:'.count($row));
		}
	...
	public static function GetMatchingRowKeys($csv, $match, $fieldNames=false){
		...
		if((!key_exists('rows', $csv))||(!is_array($csv['rows'])))throw new Exception('Invalid $csv[\'rows\']');
		
		foreach($csv['rows'] as $index=>$row){
			$row_a=EasyCsv::_combine($fieldNames, $row);
			$true=true;
			foreach($match as $k=>$v){
				if($row_a[$k]!=$v){ $true=false; break; }
			}
			if($true)$keys[]=$index;
		}
